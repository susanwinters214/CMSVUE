<template>
    <section class="contact-divider-area" :style="{ backgroundColor: '#f6f7fc' }">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-md-9 col-lg-8">
                    <div class="contact-divider-content" data-aos="fade-up" data-aos-duration="1100">
                        <div class="section-title">
                            <div class="subtitle-content">
                                <img src="/images/shape/line1.png" alt="shape">
                                <h5>Contact Us.</h5>
                            </div>
                            <h2 class="title">Contact Us For <span class="bottom-style">Web Design</span> Services</h2>
                        </div>
                    </div>
                </div>
                <div class="col-md-3 col-lg-4 text-left text-md-right">
                    <nuxt-link to="/contact" class="btn btn-theme" data-aos="fade-up" data-aos-duration="1100">Contact Now <i class="icon icofont-long-arrow-right"></i></nuxt-link>
                </div>
            </div>
        </div>
        <div class="layer-shape">
            <img class="shape-style1" src="/images/shape/15.png" alt="shape">
        </div>
    </section>
</template>

<script>
    export default {

    };
</script>